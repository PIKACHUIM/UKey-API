...
{
	CK_BBOOL b = ((CK_BBOOL *)(attr.pValue))[0];
	if(b)
		return "TRUE";
	else
		return "FALSE";
...
// string.
		...
		case CKA_APPLICATION:
			{
				CString strUTF8Label = parse_string_val(attr);

				int nLen = AppStringToUtf8String((LPCTSTR)strUTF8Label, strUTF8Label.GetLength(), NULL);
				vector<char> vLabel(nLen+1, 0);
				Utf8StringToAppString((LPCTSTR)strUTF8Label, strUTF8Label.GetLength(), &vLabel[0]);
 				strRet += &vLabel[0];
